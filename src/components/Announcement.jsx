...
const Announcement = () => {
  ...
  const handleAnnouncement = async () => {
    try {
      await axios
        .post(
          "https://dgs-backend-yo9v.onrender.com/api/v1/announcement/add_announcement",
  ...
  const getAnnouncement = async () => {
    try {
      const res = await axios.get(
        "https://dgs-backend-yo9v.onrender.com/api/v1/announcement/get_announcement"
      );
  ...
  return (
    <div className="pt-10 lg:pt-0">
      <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
        <AlertDialogTrigger className="flex float-right gap-3 px-10 py-2 text-xl text-white bg-green-900 rounded-md">
          Add <Megaphone />
        </AlertDialogTrigger>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <textarea
                ...
                onChange={(e) => setAnnouncement_details(e.target.value)}
              ></textarea>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {announcement.map((item, index) => {
        return (
          <div className="flex flex-col mt-3" key={item.id}>
            <p>
              {index + 1}.{item.announcement_details}
            </p>
          </div>
        );
      })}
